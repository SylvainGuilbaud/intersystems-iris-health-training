...
#
# Usage: ./adm_utils.sh <environment> <command> [arguments]
#        ./adm_utils.sh <command>          # defaults to local-dev
#
...
#   messages [day] - Count MLLP/HTTP messages for a target day (0=today, 1=yesterday, 7=a week ago; default 0 ; 
#                    supports ranges like 7-0 for last week 
#                    and 7* for last week up to today
#                    or *7 for all days before 7 days ago
#   audit [n] - Last n audit events (with descriptions) per Security.Users user; default 5
#   listusers - List all users with their enabled state and roles
#   setup   - Create/update interop security profiles (roles + test users)
#
# Examples:
#   ./adm_utils.sh list                      # local-dev list
#   ./adm_utils.sh local-dev recover         # local dev recover
#   ./adm_utils.sh community-prod start       # community prod start
#   ./adm_utils.sh aws-dev start             # AWS dev start
#   ./adm_utils.sh aws-prod stop             # AWS prod stop
...
REPORTS_DIR="./reports"
REPORT_FILE="${REPORTS_DIR}/adm_utils_$(date +%Y%m%d_%H%M%S).md"
...
EOF
    ...
    # For audit: show how many recent events per user are reported
    if [ "${command}" = "audit" ]; then
        local nev="${COMMAND_ARG:-5}"
        cat >> "${REPORT_FILE}" << EOF
| Property | Value |
|----------|-------|
| **Events per user** | ${nev} most recent |
...
```bash
EOF
    echo "./adm_utils.sh ${environment} ${command} ${COMMAND_ARG}" >> "${REPORT_FILE}"
    echo '```' >> "${REPORT_FILE}"
...
---
*Generated by adm_utils.sh at $(date '+%Y-%m-%d %H:%M:%S')*
EOF
...
generate_namespaces_table() {
    ...
    if [ "${command}" = "audit" ]; then
        # Audit report: User / Last Action / last N events with descriptions.
        # Each event date (text before " :: ") is colored green via inline HTML.
        jq -r '.namespaces[] |
            (.actions | map(
                (split(" :: ")) as $p |
                (if ($p | length) > 1
                 then "<span style=\"color:#a5f3fc\">" + $p[0] + "</span> :: " + ($p[1:] | join(" :: "))
                 else . end)
             ) | join("<br>")) as $events |
            "| \(.namespace) | \(if .status == "never" then "🚫 never" else "<span style=\"color:#9ef01a\">" + .status + "</span>" end) | \(if (.actions | length) == 0 then "—" else $events end) |"' \
            "${json_file}" 2>/dev/null | \
        {
            echo "| User | Last Action | Recent Events |"
            echo "|------|-------------|---------------|"
            cat
        }
    elif [ "${command}" = "listusers" ]; then
        # User listing: User / Enabled / Roles
        jq -r '.namespaces[] |
            "| \(.namespace) | \(if .status == "enabled" then "✅ enabled" elif .status == "disabled" then "⛔ disabled" else .status end) | \(if (.roles // "") == "" then "—" else .roles end) |"' \
            "${json_file}" 2>/dev/null | \
        {
            echo "| User | Enabled | Roles |"
            echo "|------|---------|-------|"
            cat
        }
    else
    ...
    echo ""
    if [ "${command}" = "audit" ]; then
        local summary=$(jq -r '.summary | "**Summary**: \(.total) users"' "${json_file}" 2>/dev/null)
    elif [ "${command}" = "listusers" ]; then
        local summary=$(jq -r '.summary | "**Summary**: \(.total) users"' "${json_file}" 2>/dev/null)
    else
...
print_help() {
    ...
    echo "  audit [n] - Last n audit events (with descriptions) per Security.Users user; default 5"
    echo "  listusers - List all users with their enabled state and roles"
    echo "  setup   - Create/update interop security profiles (roles + test users)"
    echo ""
    echo "Examples:"
    echo "  $0 list"
...
validate_command() {
    case "$1" in
        list|recover|start|stop|clean|messages|audit|listusers|setup)
            return 0
            ;;
        *)
            echo -e "${RED}ERROR: Unknown command '$1'${NC}"
            echo "Valid commands: list, recover, start, stop, clean, messages, audit, listusers, setup"
            return 1
            ;;
    esac
}
...
main() {
    ...
    # Build a filename-safe slug from command + arguments + environment so reports
    # are easy to identify, e.g. adm_utils_aws-dev_audit_10_20260629_163603.md
    local ARG_SLUG=$(echo "${COMMAND_ARG}" | tr -c 'A-Za-z0-9' '_' | sed 's/_\{1,\}/_/g; s/^_//; s/_$//')
    local NAME_SLUG="${ENVIRONMENT}_${COMMAND}"
    [ -n "${ARG_SLUG}" ] && NAME_SLUG="${NAME_SLUG}_${ARG_SLUG}"
    local STAMP=$(date +%Y%m%d_%H%M%S)
    REPORT_FILE="${REPORTS_DIR}/adm_utils_${NAME_SLUG}_${STAMP}.md"
    REPORT_OUTPUT_FILE="${REPORTS_DIR}/output_${NAME_SLUG}_${STAMP}.json"
    ...
    local OUTPUT_FILE="${REPORT_OUTPUT_FILE:-${REPORTS_DIR}/output_$(date +%Y%m%d_%H%M%S).json}"
